...
void main() {
    ...
    if (zDepth > scene.nearClip) {
        zSlice = uint(log(zDepth / scene.nearClip) * float(scene.gridZ) / log(scene.farClip / scene.nearClip));
    }
    ...
    uint ySlice = uint(gl_FragCoord.y / (scene.screenHeight / float(scene.gridY)));
    
    xSlice = min(xSlice, uint(scene.gridX - 1));
    ySlice = min(ySlice, uint(scene.gridY - 1));
    
    uint clusterIdx = xSlice + (ySlice * scene.gridX) + (zSlice * scene.gridX * scene.gridY);
    clusterIdx = min(clusterIdx, uint(scene.gridX * scene.gridY * scene.gridZ - 1));

    ClusterGrid grid = allClusterGridBuffers[nonuniformEXT(scene.clusterGridBufferIndex)].grids[clusterIdx];

    for (uint i = 0; i < grid.count; i++) {
        uint lightIdx = allLightIndexBuffers[nonuniformEXT(scene.clusterLightIndexBufferIndex)].indices[grid.offset + i];
